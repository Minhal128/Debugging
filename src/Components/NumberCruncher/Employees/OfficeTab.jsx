...
import { EditableRow, EditableCell } from "../../EditableCell/EditableCell";
...
const OfficeTab = () => {
  ...
  const [fetchedPercentValue, setFetchedPercentValue] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Handle employees
      if (employeeResponse.meta.requestStatus === "fulfilled") {
        console.log("Employees fetched successfully");
        if (employeeResponse.payload?.length > 0) {
          const officeEmployees = employeeResponse.payload.filter((emp) => emp.employeeType === "office");
          setFilteredEmployees(officeEmployees);

          if (officeEmployees.length > 0) {
            const firstEmployeeId = officeEmployees[0].id;
  ...
  const handleEmployeeSelect = (value) => {
    ...
    if (selectedEmployee) {
      ...
      // Log the fetched hourlyRate value
      console.log("Fetched hourlyRate value:", selectedEmployee.hourlyRate || "");
      ...
      // Extract the percent value from the first tax benefit's levels
      if (selectedEmployee.taxBenefits.length > 0 && selectedEmployee.taxBenefits[0].taxBenefit.levels.length > 0) {
        const firstTaxBenefit = selectedEmployee.taxBenefits[0].taxBenefit;
        const percentValue = firstTaxBenefit.levels[0].percent;
        setFetchedPercentValue(percentValue); // Store the fetched percent value
        console.log("Fetched percent value:", percentValue);

        // Calculate the calculated hourly value
        const hourlyRate = parseFloat(selectedEmployee.hourlyRate) || 0;
        const calculatedHourly = ((hourlyRate * percentValue) / 100).toFixed(3); // Calculate the hourly value

        // Calculate the annual cost
        const payHours = parseFloat(selectedEmployee.payHours) || 0; // Get pay hours
        const annualCost = ((calculatedHourly * percentValue * 52) / 100).toFixed(2);

        // Update the table data with the calculated annual cost
        const updatedTableData = newTableData.map((row) => ({
          ...row,
          annualCost: annualCost, // Set the calculated annual cost
          hourly: calculatedHourly, // Set the calculated hourly value
        }));
        setTableData(updatedTableData); // Update the state with the new table data
      }
    }
  };
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      // Calculate hourlyRate if annualPay is changed
      ...
      // Calculate annualPay if hourlyRate is changed
  ...
  const handlePayHoursChange = (e) => {
    const value = e.target.value;
    setFormData((prev) => {
      const updatedData = { ...prev, payHours: value };

      if (updatedData.annualPay) {
        updatedData.hourlyRate = (parseFloat(updatedData.annualPay) / (parseFloat(value) * 52)).toFixed(2) || "";
      }
      if (updatedData.hourlyRate) {
        updatedData.annualPay = (parseFloat(updatedData.hourlyRate) * (parseFloat(value) * 52)).toFixed(2) || "";
      }
  ...
  const handleSubmit = async (e) => {
    ...
    // Use formData.employeeType directly
        ...
        const updatedRows = prev.map((row) => {
          if (row.key === key) {
            ...
            if (field === "taxBenefit") {
              ...
              // Store the percent value in the row
              updatedRow.percent = percentValue;

              // Calculate hourly based on the fetched percent value and hourlyRate
              const hourlyRate = parseFloat(formData.hourlyRate) || 0; // Use the hourlyRate from formData
              const calculatedHourly = ((hourlyRate * percentValue) / 100).toFixed(3); // Updated calculation
              updatedRow.hourly = calculatedHourly; // Store the calculated hourly value

              // Calculate annualCost based on the calculated hourly value
              const payHours = parseFloat(formData.payHours) || 0;
              updatedRow.annualCost = (calculatedHourly * fetchedPercentValue * 100 * payHours * 52).toFixed(2); // Calculate annual cost

              // Log the calculation
              console.log(`Calculating annual cost for ${value}:`);
              console.log(
                `Hourly Rate: ${hourlyRate}, Percent Value: ${percentValue}, Calculated Hourly: ${updatedRow.hourly}, Annual Cost: ${updatedRow.annualCost}`
              );

              // Store the taxBenefitId
  ...
  const handleDeleteTaxBenefit = async (taxBenefitId) => {
    try {
      // Dispatch the action to delete the tax benefit by ID
      const response = await dispatch(EmployeeTaxBenefitDeleteByID(taxBenefitId));
      if (response.meta.requestStatus === "fulfilled") {
        message.success("Tax benefit deleted successfully");
        // Update the table data to remove the deleted tax benefit
        setTableData((prev) => prev.filter((row) => row.taxBenefitId !== taxBenefitId));
      } else {
  ...
  // Update the action column to pass the correct taxBenefitId
  const column1 = [
    {
    ...
    },
    {
      ...
      render: (text, record) => (
        <EditableCell
          title="Annual Cost"
          editable
          children={<Input value={text} onChange={(e) => handleRowChange(record.key, "annualCost", e.target.value)} />}
          handleSave={handleRowChange}
          record={record}
          dataIndex="annualCost"
        />
      ),
    ...
    },
    {
      ...
      render: (text, record) => (
        <EditableCell
          title="Hourly"
          editable
          children={<Input value={text} onChange={(e) => handleRowChange(record.key, "hourly", e.target.value)} />}
          handleSave={handleRowChange}
          record={record}
          dataIndex="hourly"
        />
      ...
      render: (text, record) => {
        return (
          <div className="flex justify-center">
            ...
            <Button
              type="text"
              onClick={() => handleDeleteTaxBenefit(record.taxBenefitId)}
    ...
    <div className="p-6 w-full">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Select className="h-8 w-40 border border-gray-300 rounded-[10px]" onChange={handleEmployeeSelect}>
      ...
      <div className="flex space-x-6 w-full">
        <div className="w-1/3 pr-3">
          <Form onSubmitCapture={handleSubmit} layout="vertical" className="space-y-4">
            <Form.Item label={<span className="font-medium text-gray-600">Last Name:</span>}>
              <Input
                name="lastName"
            ...
            <Form.Item label={<span className="font-medium text-gray-600">Employee Type:</span>}>
              <Select
                value={formData.employeeType}
                onChange={(value) => handleSelectChange("employeeType", value)}
                style={{ width: "70%" }}
                className="font-semibold"
              >
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="space-x-2 mt-2 font-semibold" style={{ marginRight: "35px" }}>
                <Checkbox name="hourly" checked={formData.hourly} onChange={handleCheckboxChange} />
                <span>Hourly</span>
              </div>
            </div>

            <div className="flex items-center space-x-4" style={{ marginTop: "-5px", marginBottom: "-15px" }}>
